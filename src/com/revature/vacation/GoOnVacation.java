...
public class GoOnVacation {
	...
	{
		Vehicle[] vehicles = {new Tornado(147.2), new Kayak("red",4.2,2,2), new Car(2021, "spaceship", "Tesla", 50), new Tornado(256.3)};
	...
	static void vacationTime()
	{
		...
		System.out.println(c.toString()); //prints out whatever toString() returns
		System.out.println("do we need to change the oil first?");
		System.out.println("recommened miles between oil changes: "+Car.recommendedMiBtwnOilChanges);
		
		if(c.getMilesSinceOilChange() >= recommendedMiBtwnOilChanges)
		{
